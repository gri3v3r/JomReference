...
if (!isset($_POST['uid'])) die ('Error'); // uid , user who requests permission 
...
if (!isset($_POST['lck']) || intval($_POST['lck'])!=1) die ('Restricted access'); // restrict access on insecure call
  ...
  function givePermission(){
    ...
    if($user->id!=0){
	 ...
	 $database->setQuery( "update `#__jomreference_permissions`  set `accept`=1 where `sourceid`=".intval($_POST['uid'])." and `targetid`=".$user->id);
	 $database->query();	
    }
   }
